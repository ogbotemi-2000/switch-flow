...
const Ev = ev=>node=>(cb, arg)=>/^on/.test(ev)?node[ev]=cb:node[node.attachEvent?'attachEvent':'addEventListener'](ev, cb, arg),
      ...
      debug=/debug/.test(location.search), slice=arr=>[].slice.call(arr), qsa=str=>slice(document.querySelectorAll(str)),
      uuid   = (a) => (a ? (a ^ ((Math.random() * 16) >> (a / 4))).toString(16) : ([1e7] + -1e3 + -4e3 + -8e3 + -1e11).replace(/[018]/g, uuid)),
      crE = (tag, attrs, values)=>(tag = document.createElement(tag), values&&Object.keys(values).forEach(key=>tag[key]=values[key]), attrs.forEach(attr=>tag.setAttribute(attr.name, attr.value)), tag);

      debug&&w_Ev('onbeforeunload')(_=>'Prevents navigation or reload of page without user-prompted consent');

(function(w, aside, grid, ctx) {
  /**accessinig elements directly via their id, works due to legacy browser support*/
  w_Ev_dom(_=>{ ctx = (grid = w['grid-canvas']).getContext('2d'), aside=document.querySelector('body>switchFlow>aside') })

  /** redraw everytime when called */
  Grid = (glyph, font, h) =>requestAnimationFrame(_=>{
    ctx.clearRect(0, 0, grid.width, grid.height),
    glyph = Grid.pattern||='.', font = (Grid.size||=12)+'px Arial',
    ctx.font = font, ctx.fillStyle = 'transparent', ctx.fillRect(0, 0, grid.width, h=grid.height),
    ctx.fillStyle = '#000', ctx.fillText(glyph, 0, h),
    aside&&(aside.style.background = `url(${grid.toDataURL('image/x-windows-bmp')})`)
  }), w.Grid=Grid
})(window)
...
function grow_shrink(e,i,c,n,d,k, cls){
  ...
  /*d.vw!==e&&!d.cached[*/(d.vw=e)/*]*/&&d.arr.forEach((n,r,o)=>{
    ...
    if(((cls=n.classList)+'').match(k)) cls.remove('clicked'), (cls+'').replace(/(base|sm|md|lg|xl):show/, function(a) {
      cls.remove(a, 'fluid')
    }), c.appendChild(n), !d.cached[e]&&(d.cached[e]=c)
...
function relation(parent, child) {
  ...
  Abbr.dict(['previousSibling', 'textContent', 'childNodes', 'classList', 'parentNode', 'previousElementSibling', 'nextElementSibling', 'nextSibling', 'firstChild', 'firstElementChild', 'lastChild', 'lastElementChild']);
